 ...
 * Define Constants
 */
define('KSASCENT_SHORTNAME', 'ksascent'); // used to prefix the individual setting field id see ksascent_options_page_fields()
define('KSASCENT_PAGE_BASENAME', 'ksascent-settings'); // the settings page slug

/**
 * Specify Hooks/Filters
 */
add_action( 'admin_menu', 'ksascent_add_menu' );
add_action( 'admin_init', 'ksascent_register_settings' );
...
require_once('ksascent-theme-options.php');
function ksascent_get_settings_page_cap() {
	return 'edit_theme_options';
}
// Hook into option_page_capability_{option_page}
add_filter( 'option_page_capability_ksascent_options', 'ksascent_get_settings_page_cap' );
 ...
 * @return array
 */
function ksascent_get_settings() {
	
	$output = array();
	
	// put together the output array 
	$output['ksascent_option_name'] 		= 'ksascent_options'; // the option name as used in the get_option() call.
	$output['ksascent_page_title'] 		= __( 'Centers/Programs Theme Options','ksascent_textdomain'); // the settings page title
	$output['ksascent_page_sections'] 	= ksascent_options_page_sections(); // the setting section
	$output['ksascent_page_fields'] 		= ksascent_options_page_fields(); // the setting fields
 ...
 */
function ksascent_create_settings_field( $args = array() ) {
	...
	$defaults = array(
		...
		'section' => 'main_section', 					// the section this setting belongs to — must match the array key of a section in ksascent_options_page_sections()
	...
	// additional arguments for use in form field output in the function ksascent_form_field_fn!
	$field_args = array(
		'type'      => $type,
		'id'        => $id,
		'desc'      => $desc,
	...
	add_settings_field( $id, $title, 'ksascent_form_field_fn', __FILE__, $section, $field_args );
 ...
 */
function ksascent_register_settings(){
	
	// get the settings sections array
	$settings_output 	= ksascent_get_settings();
	$ksascent_option_name = $settings_output['ksascent_option_name'];
	
	//setting
	register_setting($ksascent_option_name, $ksascent_option_name, 'ksascent_validate_options' );
	
	//sections
	if(!empty($settings_output['ksascent_page_sections'])){
		// call the "add_settings_section" for each!
		foreach ( $settings_output['ksascent_page_sections'] as $id => $title ) {
			add_settings_section( $id, $title, 'ksascent_section_fn', __FILE__);
		}
	}
		
	//fields
	if(!empty($settings_output['ksascent_page_fields'])){
		// call the "add_settings_field" for each!
		foreach ($settings_output['ksascent_page_fields'] as $option) {
			ksascent_create_settings_field($option);
		}
	}
}
 ...
 * The Admin menu page
 */
function ksascent_add_menu(){

	$settings_output 		= ksascent_get_settings();
	
	// Display Settings Page link under the "Appearance" Admin Menu
	// add_theme_page( $page_title, $menu_title, $capability, $menu_slug, $function);
	$ksascent_settings_page = add_theme_page(__('Centers/Programs Theme Options'), __('Centers/Programs Theme Options','ksascent_textdomain'), 'edit_theme_options', KSASCENT_PAGE_BASENAME, 'ksascent_settings_page_fn');
		// contextual help
		// css & js
		add_action( 'load-'. $ksascent_settings_page, 'ksascent_settings_scripts' );	
}
 ...
 */
function  ksascent_section_fn($desc) {
	echo "<p>" . __('Settings for this section','ksascent_textdomain') . "</p>";
}
 ...
 * @return echoes output
 */
function ksascent_form_field_fn($args = array()) {
	...
	$ksascent_option_name = $settings_output['ksascent_option_name'];
	$options 			= get_option($ksascent_option_name);
	...
	switch ( $type ) {
		case 'text':
			...
			echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $ksascent_option_name . "[$id]' value='$options[$id]' />";
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				$item[0] = esc_html__($item[0], 'ksascent_textdomain');
				if (!empty($options[$id])) {
				...
				echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $ksascent_option_name . "[$id|$item[1]]' value='$value' /><br/>";
			}
		...
		case 'textarea':
			...
			echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $ksascent_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
		
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $ksascent_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'ksascent_textdomain');
					$item 	= esc_html($item, 'ksascent_textdomain');
		...
		case 'select2':
			...
			foreach($choices as $item) {
				...
				$item = explode("|",$item);
				$item[0] = esc_html($item[0], 'ksascent_textdomain');
		...
		case 'checkbox':
			echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $ksascent_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
		...
		case "multi-checkbox":
			foreach($choices as $item) {
				...
				echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $ksascent_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
			}
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
	}
 ...
 * @return echoes output
 */
function ksascent_settings_page_fn() {
	// get the settings sections array
	$settings_output = ksascent_get_settings();
?>
	<div class="wrap">
		<div class="icon32" id="icon-options-general"></div>
		<h2><?php echo $settings_output['ksascent_page_title']; ?></h2>
		
		<form action="options.php" method="post">
			<?php 
			// http://codex.wordpress.org/Function_Reference/settings_fields
			settings_fields($settings_output['ksascent_option_name']); 
 ...
 * @return array
 */
function ksascent_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['ksascent_page_fields'];
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									KSASCENT_SHORTNAME . '_txt_numeric_error', // error ID
									__('Expecting a Numeric value! Please fix.','ksascent_textdomain'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','ksascent_textdomain');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									$option['id'], // setting title
									KSASCENT_SHORTNAME . '_txt_multinumeric_error', // error ID
									__('Expecting comma separated numeric values! Please fix.','ksascent_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','ksascent_textdomain');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','ksascent_textdomain');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									$option['id'], // setting title
									KSASCENT_SHORTNAME . '_txt_email_error', // error ID
									__('Please enter a valid email address.','ksascent_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
 ...
 */
function ksascent_show_msg($message, $msgclass = 'info') {
